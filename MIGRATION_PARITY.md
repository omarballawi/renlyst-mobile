...
- [x] Reproducible local release verification exports both iOS and Android Hermes bundles before the manually dispatched unsigned iOS packaging workflow.
...
- 2026-07-19: hosted simulator and Maestro work were removed from the user-requested fast release workflow. The manual macOS workflow now builds only the unsigned device app, verifies `Payload/*.app/Info.plist` and SHA-256, and uploads a versioned IPA. Native interaction acceptance remains a separate device task rather than a packaging blocker.
